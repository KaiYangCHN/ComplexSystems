...
[ 
  ...
  connected?                    ;; if true, the node is connecting with another node,the virus is being delivered  
  recovered?                    ;; if true, the node can't be infected
  target                        ;; an agent, record which node is connected with the infected node 
  step                          ;; number, record how much time has used to transfer the file
]
...
to initialize
  ifelse initial-outbreak > number-of-nodes     ;; initial outbreak nodes can't more than total nodes
  [ show "initial outbreak number can't be bigger than the number of nodes" stop ]
  [ ask n-of initial-outbreak turtles [ become-infected ] ]
end

to go
  if count turtles with [infected?] = 0
  [ stop ]
  move                                           ;; nodes start to move
  spread-virus                                   ;; infected nodes start to spread virus
  recover                                        ;; node recovers,can't be infected again
  check                                          ;; check the transfer and connection states 
  tick
end

;; check if the transfer is completed or if the distance between two connected nodes is bigger than the radius 
to check
  ask turtles with [infected? and connected?]
  [ ;; how much time the transfer has used, in order to observe the spreading better,
    ;; by doubling the needed time of the transfer to slow the spreading speed
    ifelse step < time * 2
    [ ifelse distance target < radius              ;; the node selected is still in the coverage area
      [ set step (step + 1) ]
      [ ask target [ become-not-connected ]        ;; the node selected moves out of the coverage area, transfer is interrupted
        ask link who [who] of target [ die ]
        become-not-connected        
      ]
    ]
    [ if random-float 100 < install-chance
      [ ask target [ become-infected ]             ;; transfer is completed, users install the malware, devices are infected
        ask link who [who] of target [ die ]
        become-not-connected           
      ]
    ]
...
to move
  ask turtles
  [ fd speed
    ;; when nodes move near to the margins, change direction
    if [xcor] of self > 19 or [xcor] of self < -19 or [ycor] of self > 19 or [ycor] of self < -19
    [ rt random 360 
      fd 1
    ]
  ]
...
;; find a node to send the virus
to spread-virus  
  ask turtles with [infected? and not connected?]
  [ let choice one-of other turtles in-radius radius           ;; can connect with only one node once time
    ifelse choice != nobody and [ susceptible? ] of choice and random-float 100 < accept-chance
    [ ask choice [ become-connected ]                          ;; the node selected is connected
      become-connected                                         ;; connects with another node
      create-link-with choice
      ask links [ set color white ]
      set target choice                                        ;; set target as the node just connected
      set step (step + 1)                                      ;; starts to transfer virus 
    ]
    [ stop ]
  ]
end
...
"connected" 1.0 0 -1184463 true "" "plot count turtles with [connected?]"
